...
function LogicInit(tel,id) {
    setTimeout(function () {
        ...
        if (errorcode == GV_ERR_SUCCESS) {
			...
			if(mRefreshPluginTimer == -1) {
			    mRefreshPluginTimer = setInterval(function () { LogicInit(tel, id); }, 1000);
			}
		}
    }, 500);
}
...
function ShowHallDiv(bShow) {
	...
	//	GetID("login_div").style.display = "none"; 		//隐藏登录界面
		GetID("zhuanjia").style.display = "block"; 		//显示大厅界面
		SetDivTop("zhuanjia", 400); 					//大厅界面垂直居中
	} else {
		GetID("zhuanjia").style.display = "none";
	}
}
...
function initialize() {
    ...
    CenterPosition(mBrowserWidth, mBrowserHeight, "zhuanjia", 770, 650); //结束会话询问层垂直水平居中
    if (mBrowserHeight < 650) $("#zhuanjia").css("top", "0px");
}
