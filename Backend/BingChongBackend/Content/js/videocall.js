...
function CreateUserImage(type) {
    ...
    if (type == "whole") { // 所有在线用户
        ...
        for (var i = 0; i < OnLineUserList.length; i++) {
            ...
            {  
				  //alert(OnLineUserList[i]);
				for(var j=0;j<idlist.length;j++)
				{	
					if(OnLineUserList[i]==idlist[j])
					{
...
function DisplayOnLineUser(userid) {
	...
	var UserIp=BRAC_GetUserInfo(userid,USERINFO_IP); // 用户IP地址;
			...
			left_img.src = rootUri+"content/img/head/"+img_value+".gif";
			...
			right_ip.className = "UserInfo";//13848935710
...
function ForSession(message) {
    ...
    Getdmo("show_shipin").style.display = "none"; 				// 隐藏设置层
	mTargetUserId=0;
...
function onSendVideoCallRequestSucess(mTargetUserId)
{
    ...
    Getdmo("Initiative_Call_Div_Content").innerHTML = "正在呼叫" + UserName + "用户，等待对方响应<br /><img src='"+rootUri+"content/img/LoadImg.gif'    style='width: 145px;height:30px;' />";
