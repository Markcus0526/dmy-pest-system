...
var mDefaultServerAddr = "120.193.233.26";		// 默认服务器地址
...
function InitInterfaceUI() {
    ...
    GetID("loginbtn").onclick = function () {
		...
		if (GetID("username").value != "") {
		    var b = false;
		    for (var i = 1; i <= LOG_CONT_MAX; i++) {
		        if (GetID("username").value == i && GetID("password").value == i) {
		            b = true;
		        }
		    }
		    if (!b) {
            ...
            AddLog("BRAC_Connect(" + GetID("ServerAddr").value + "," + GetID("ServerPort").value + ")=" + errorcode, LOG_TYPE_API);
            errorcode = BRAC_Login("zhuanjia_"+GetID("username").value, GetID("password").value, 0);
            AddLog("BRAC_Login(" + GetID("username").value + ")=" + errorcode, LOG_TYPE_API);
            // 隐藏设置界面
            GetID("setting_div").style.display = "none";
        }
...
function DisplayLoadingDiv(bShow) {
    if (bShow) {
        ...
        GetID("LOADING_DIV").style.marginTop = (TheHeight - 200) / 2 + "px";
        GetID("LOADING_DIV").style.marginLeft = (TheWidth - 300) / 2 + "px";
    }
    else {
...
function initialize() {
    ...
    CenterPosition(mBrowserWidth, mBrowserHeight, "Initiative_Call_Div", 520, 390); //主动呼叫层垂直水平居中
    CenterPosition(mBrowserWidth, mBrowserHeight, "BeCalls_Div", 520, 390); //被呼叫层垂直水平居中
	CenterPosition(mBrowserWidth, mBrowserHeight, "SessionPrompt_Div", 300, 170); //会话消息层垂直水平居中
	CenterPosition(420, 2, "advanceset_div", 0, 0); //会话消息层垂直水平居中
    CenterPosition(mBrowserWidth, mBrowserHeight, "hall_div", 770, 150); //结束会话询问层垂直水平居中
...
function showVideoSessionScreen() {
    //网页可见区域宽：
    var VWidth = document.body.offsetWidth; // (包括边线的宽) 有
    //网页可见区域高：
    var VHeight = document.body.offsetHeight; //  (包括边线的高)
    CenterPosition(VWidth, 730, "show_shipin", 1020, 450);
    createVideoContainer(); // 动态生成宽屏视频容器
    setVideoShow("videoshow_2", "videoshow_1");
...
function setVideoShow(firVideo, secVideo) {
    ...
    GetID("ANYCHAT_VIDEO_LOCAL").SetBkImage("http://120.193.233.26:8006/b/lvyun.jpg");
    BRAC_SetVideoPos(mTargetUserId, Getdmo(secVideo), "ANYCHAT_VIDEO_REMOTE");
    GetID("ANYCHAT_VIDEO_REMOTE").SetBkImage("http://120.193.233.26:8006/b/lvyun.jpg");
}
// 双方音量条
function updateCurrentVolume() {
    /*if ($("#" + dwUserId + "_CameraTag").length < 1) {
        return;
    }*/
...
function createVideoContainer() {
  /*  Getdmo("show_shipin").innerHTML = "";
   ...
   /* var upper_othername = document.createElement("div");
    ...
    Getdmo("show_shipin").appendChild(upper_othername);
    // 显示对方名字
    ...
    Getdmo("show_shipin").appendChild(upper_myname);
    // 挂断 按钮
    ...
    Getdmo("show_shipin").appendChild(under_finish);
    // 高级设置 按钮
   ...
   video_paramers_config.onclick = function () {
    ...
    Getdmo("show_shipin").appendChild(video_paramers_config);*/
}
