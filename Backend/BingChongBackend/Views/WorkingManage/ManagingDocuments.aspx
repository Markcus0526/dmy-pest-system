		...
		<div class="pull-right">
            <p>
	            <%if (ViewData["role"] != null && ((String)ViewData["role"]).Contains("AddManagementMethod"))
                   {%>
                   <a class="btn btn-sm btn-white  btn-default btn-bold" href="<%= ViewData["rootUri"] %>WorkingManage/AddDocuments">
...
<asp:Content ID="Content3" ContentPlaceHolderID="PageScripts" runat="server">
    ...
    <script type="text/javascript">
        jQuery(function ($) {
            ...
            $("#myDataTable").jqGrid({
                // multiselect: true, //复选
                url: '<%= ViewData["rootUri"] %>' + 'WorkingManage/RetrieveDocumentsList',
                datatype: "json",
                height: 313,
              //  width: 1000,
                autowidth:true,
                ...
                colNames: ['标题', '发布者', '所在地区', '发布时间','操作'],
                colModel: [
                            { name: 'title', index: 'title', align: "center" },
                            { name: 'creatername', index: 'creatername', align: "center" },
                            { name: 'regionname', index: 'regionname', align: "center" },
                            { name: 'creattime', index: 'creattime', align: "center" },
                            { name: 'uid', index: 'uid', align: "center",
                                formatter: function (cellvalue, options, rowObject) {
                                var html="";
                                    <%if (ViewData["role"] != null && ((String)ViewData["role"]).Contains("ViewManagementMethod")) {%>
                                    html+='<a  href=" <%= ViewData["rootUri"] %>WorkingManage/ViewDocument/' + cellvalue + '"  style="font-size:12px">查看</a>&nbsp;&nbsp;';
                                    <%} %>
                                    <%if (ViewData["role"] != null && ((String)ViewData["role"]).Contains("UpdateManagementMethod")) {%>
                                    html+='<a  href=" <%= ViewData["rootUri"] %>WorkingManage/EditDocument/' + cellvalue + '"   style="font-size:12px">修改</a>&nbsp;&nbsp;';
                                    <%} %>
                                    <%if (ViewData["role"] != null && ((String)ViewData["role"]).Contains("DeleteManagementMethod")) {%>
        ...
        function processDel(sel_id) {
            ...
            if (selected_id != "") {
                ...
                if (con == true) {
                    $.ajax({
                        url: '<%= ViewData["rootUri"] %>' + 'WorkingManage/DeleteDocument',
                        data: {
                            "delids": selected_id
                        },
                        type: "post",
