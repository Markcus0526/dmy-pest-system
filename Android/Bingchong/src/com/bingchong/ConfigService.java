...
public class ConfigService {
	...
	public static ConfigEntity LoadConfig(Context context)
	{
        ...
        //configEntity.ip = share.getString("ip", configEntity.ip);
        //configEntity.ip = share.getString("ip", configEntity.ip);
        ...
        configEntity.videoBitrate = share.getInt("videoBitrate", 60*1000);
        configEntity.videoFps = share.getInt("videoFps", 10);
        configEntity.videoQuality = share.getInt("videoQuality", ConfigEntity.VIDEO_QUALITY_NORMAL);
	...
	public static int LoadQuality(Context context){
		int quality = 0;
		SharedPreferences  share = context.getSharedPreferences("save_quality", Context.MODE_WORLD_WRITEABLE);  
		quality = share.getInt("quality", 0);
		return quality;
	}
	
	
	@SuppressWarnings("deprecation")
	public static void SaveQuality(Context context,int quality){
		SharedPreferences  share = context.getSharedPreferences("save_quality",  Context.MODE_WORLD_WRITEABLE);  
        Editor editor = share.edit();//取得编辑器
