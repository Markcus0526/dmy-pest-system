...
//import android.net.http.AndroidHttpClient; //above android 2.2 
...
public class ImageDownloader {
	...
	public ImageDownloader(Context paramContext) {
		...
		this.IMAGE_WIDTH = mWindowManager.getDefaultDisplay().getWidth()/6;
		this.IMAGE_HEIGHT = mWindowManager.getDefaultDisplay().getHeight()/8;
	}
	...
	public void download(String url, ImageView imageView) {
		...
		file_download(url, imageView, null, mwidth, mheight);
	}
	
	public void download(String url, ImageView imageView, int width, int height) {
		mwidth = width;
		mheight = height;	
		file_download(url, imageView, null, width, height);
	}
	 ...
	 */
	public void file_download(String url, ImageView imageView, String cookie, int width, int height ) {
		resetPurgeTimer();
		...
		if (bitmap == null) {
			...
			Bitmap zoomBitmap = ImageUtil.zoomBitmap(bitmap, width,	height);
	...
	private void forceDownload(String url, ImageView imageView, String cookie) {
		...
		if (cancelPotentialDownload(url, imageView)) {
			...
			DownloadedDrawable downloadedDrawable = new DownloadedDrawable(task, ImageUtil.drawableToBitmap(ctx.getResources().getDrawable(R.drawable.person)));
	...
	class BitmapDownloaderTask extends AsyncTask<String, Void, Bitmap> {
		...
		protected Bitmap doInBackground(String... params) {
			...
			// AndroidHttpClient.newInstance("Android");//above android 2.2
			final DefaultHttpClient httpClient = defaultHttpClient();
			url = params[0];
			
			try {
				HttpPost httpRequest = new HttpPost(url);
				HttpParams param = new HttpParams();
				param.addParam(Constant.POST_DOWN_IMAGE_WIDTH, Integer.toString(mwidth));
				param.addParam(Constant.POST_DOWN_IMAGE_HEIGHT, Integer.toString(mheight));			
				httpRequest.setEntity(new UrlEncodedFormEntity(param.getParams(), HTTP.UTF_8));
				...
				if (entity != null) {
					...
					try {
						...
						// kimgi - outof memory error
						final Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
						if (bitmap == null) {
							final Bitmap defafultbitmap = ImageUtil.drawableToBitmap(ctx.getResources().getDrawable(R.drawable.person));
							return defafultbitmap;
						}
						final Bitmap zoomBitmap = ImageUtil.zoomBitmap(bitmap,
								mwidth, mheight);
						final Bitmap roundBitmap = ImageUtil
								.getRoundedCornerBitmap(zoomBitmap, 6.0f);
				...
				if (httpClient != null) {
					// client.close(); //above android 2.2
				}
			}
			return null;
		}
	...
	static class DownloadedDrawable extends Drawable {
		...
		public DownloadedDrawable(BitmapDownloaderTask bitmapDownloaderTask, Bitmap b) {
			mBitmap = b;
			//super(R.drawable.bg_default);
