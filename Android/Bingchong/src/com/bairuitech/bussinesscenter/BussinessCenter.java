...
import com.bingchong.activity.video.VideoActivity;
...
public class BussinessCenter{
	...
	public static void VideoCallControl(int dwEventType, int dwUserId, int dwErrorCode, int dwFlags, int dwParam, String szUserStr) {
		Log.e("call",dwEventType+"");
		Log.e("call",dwUserId+"");
		Log.e("call",dwErrorCode+"");
		Log.e("call",dwFlags+"");
		Log.e("call",dwParam+"");
		Log.e("call",szUserStr+"");
	...
	public void onVideoCallRequest(int dwUserId, int dwFlags,
		...
		if (bBack) {
			...
			BaseMethod.sendBroadCast(mContext, Constant.ACTION_BACK_EQUESTSESSION, bundle);
		}
	}
		...
		if (strMessage != null) {
			...
			if (bBack) {
				...
				BaseMethod.sendBroadCast(mContext,
						Constant.ACTION_BACK_CANCELSESSION, null);
			}
			stopSessionMusic();
		}
	}
	...
	public void onVideoCallStart(int dwUserId, int dwFlags, int dwParam,
			String szUserStr) {
		// TODO Auto-generated method stub
		Log.e("onVideoCallStart","onVideoCallStart");
		stopSessionMusic();
		sessionItem = new SessionItem(dwFlags, selfUserId, dwUserId);
		sessionItem.setRoomId(dwParam);
		Intent intent = new Intent();
		intent.setClass(mContext.getApplicationContext(), VideoActivity.class);
	...
	public void getOnlineFriendDatas() {
		...
		for (int i = 0; i < friendUserIds.length; i++) {
			...
			Log.e("mOnlineFriendIds" , "mOnlineFriendIds == " + friendUserId);
			Log.e("mOnlineFriendIds" , "nickName == " + nickName);
		}
		
		Log.e("mOnlineFriendItems" , "mOnlineFriendItems.size == " + mOnlineFriendItems.size());
	}
	
	/**
	 * @description 视频录制
	 * @param dwTargetUserId
	 * @param bstartrecord 1为开始录制，0为停止录制
	 */
	@SuppressWarnings("static-access")
	public void videoRecord(int dwTargetUserId , int bstartrecord){
		
		if(bstartrecord == 1){
			Log.e("AnychatRecord" , dwTargetUserId + "开始录制");
		}else if(bstartrecord == 0){
			Log.e("AnychatRecord" , dwTargetUserId + "结束录制");
		}
		//视频录制
		ConfigEntity configEntity = ConfigService.LoadConfig(mContext);
		anychat.SetSDKOptionInt(AnyChatDefine.BRAC_SO_RECORD_FILETYPE, 0);
		int dwFlags = AnyChatDefine.ANYCHAT_RECORD_FLAGS_VIDEO + AnyChatDefine.ANYCHAT_RECORD_FLAGS_AUDIO + AnyChatDefine.ANYCHAT_RECORD_FLAGS_SERVER + 
				AnyChatDefine.ANYCHAT_RECORD_FLAGS_MIXVIDEO + AnyChatDefine.ANYCHAT_RECORD_FLAGS_MIXAUDIO +
				AnyChatDefine.ANYCHAT_RECORD_FLAGS_STEREO + AnyChatDefine.ANYCHAT_RECORD_FLAGS_LOCALCB ;
		String path = Environment
				.getDataDirectory().getAbsolutePath() + "/zjspyy/";
		anychat.SetSDKOptionString(AnyChatDefine.BRAC_SO_RECORD_TMPDIR, path);
		anychat.SetSDKOptionInt(AnyChatDefine.BRAC_SO_RECORD_VIDEOBR, configEntity.videoBitrate);
		anychat.StreamRecordCtrlEx(dwTargetUserId , bstartrecord, dwFlags, 0 , "record");
	}
